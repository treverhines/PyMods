...
##------------------------------------------------------------------------------
def _running_weighted_mean(values,sigma,times,dt,cut_times=[]):
  '''
  running mean filter
...
##------------------------------------------------------------------------------
def _draw_scale(basemap,ax,scale_length,quiver_scale):
  '''
  used in the view method of StationDB
  ...
  sigma_u_scale = np.array([1.5])
  sigma_v_scale = np.array([1.5])
  v_scale = np.array([0.0])
  z_scale = np.array([0.0])
  x_scale = np.array([basemap.urcrnrx/10.0])
  y_scale = np.array([basemap.urcrnry/10.0])

  ax.text(basemap.urcrnrx/10.0,
          basemap.urcrnry/15.0,
          '%s cm displacement' % (int(scale_length/10.0)),
          fontsize=16)

  ax.quiver(x_scale,y_scale,u_scale,v_scale,sigma_u_scale,sigma_v_scale,
            scale_units='xy',
            angles='xy',
            width=0.004,
            scale=quiver_scale,
...
##------------------------------------------------------------------------------
def _plot_tseries(ax,station,direction,disp_type,formats,time_range):
  '''
  used in the view method of StationDB
  '''
  # 1.0 year ticks
...
class StationDB(dict):
  ...
  def add_predicted(self,pred,context):
    '''
    IN PROGRESS
    '''
    for pidx,p in enumerate(pred):
      ...
      sta = self[sid]
      if not hasattr(sta['north'],'predicted'):
        time_no = len(sta['time'])
        sta['north']['predicted'] = np.zeros(time_no)
        sta['northstd']['predicted'] = np.zeros(time_no)
        sta['east']['predicted']  = np.zeros(time_no)
        sta['eaststd']['predicted']  = np.zeros(time_no)
        sta['vert']['predicted']  = np.zeros(time_no)
        sta['vertstd']['predicted']  = np.zeros(time_no)

      tidx, = np.nonzero(time == sta['time'])
      tidx = tidx[0]     
      if direction == 'n':
        sta['north']['predicted'][tidx] = p

      if direction == 'e':
        sta['east']['predicted'][tidx]  = p

      if direction == 'v':
        sta['vert']['predicted'][tidx]  = p
  ...
  def view(self,
    ...
    _draw_scale(self.basemap,main_ax,scale_length,quiver_scale)
    ...
    def _onpick(event):
      ...
      _plot_tseries(sub_ax1,station,'north',disp_types,ts_formats,time_range)
      _plot_tseries(sub_ax2,station,'east',disp_types,ts_formats,time_range)
      _plot_tseries(sub_ax3,station,'vert',disp_types,ts_formats,time_range)
